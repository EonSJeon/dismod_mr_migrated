...
def build_W_linear(knots: np.ndarray, ages: np.ndarray) -> np.ndarray:
    """Return an (N×K) matrix W so that W @ y is the piecewise-linear interpolation
    of knot values y at query ages."""
...
def spline(data_type: str) -> at.TensorVariable:
    ...
    if smoothing is not None:
        smooth_sigma = pm.math.clip(at.as_tensor_variable(float(smoothing)), 1e-12, np.inf)
        ...
        penalty      = 0.5 * at.sum(diffs**2 * inv_denom) / (smooth_sigma**2)
