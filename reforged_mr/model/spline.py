...
def build_W_linear(knots: np.ndarray, ages: np.ndarray) -> np.ndarray:
    """Build linear-spline design matrix W (N×K) for given knots and ages."""
    # Checked
...
def spline(data_type: str) -> at.TensorVariable:
    """Construct a linear-spline prior for mu_age over coord 'age' in pm_model."""
    # --- Setting ---
    ...
    if "age" not in pm_model.coords:
        raise ValueError(
            "coords['age'] is missing. Register it upstream via "
    ...
    if knot_dim in pm_model.coords:
        raise ValueError(
            f"coords['{knot_dim}'] already exists; "
            "spline() should be called only once per data_type."
        )

    # age coords already checked above
    ...
    if knots[0] < first_age:
        raise ValueError(
            f"First knot ({knots[0]}) cannot be before first age ({first_age})."
        )

    # --- end constraint: cannot exceed last age ---
    if knots[-1] > last_age:
        raise ValueError(
            f"Last knot ({knots[-1]}) cannot exceed last age ({last_age})."
        )
    ...
    if isinstance(raw_smooth, dict):
        ...
        raise TypeError(
            "smoothness must be dict or one of "
            "{'No Prior','Slightly','Moderately','Very'}"
        )
    ...
    gamma = pm.Normal(
        f"gamma_{data_type}", mu=0.0, sigma=10.0, dims=(knot_dim,)
    )
    exp_gamma = at.exp(gamma)

    # --- assemble mu(age) ---
    mu_age = pm.Deterministic(
        f"mu_age_{data_type}", at.dot(W, exp_gamma), dims=("age",)
    )
    ...
    if smoothing is not None:
        ...
        inv_denom = 1.0 / (
            (knots[1:] - knots[:-1]) * (knots[-1] - knots[0])
        )  # /(Δa * total length)
