...
def neg_binom(data_type, obs_data, pi, delta) -> None:
    ...
    # delta (alpha) may be a list/scalar/ndarray/tensor → unify
    if isinstance(delta, list) and len(delta) == 1:
        delta = delta[0]
    # convert numpy to tensor; if already tensor, as_tensor_variable is no-op
    alpha_all = at.as_tensor_variable(delta)

    # consider only rows with n > 0 for observed likelihood
    nonzero_idx = np.where(n_int > 0)[0]
    if nonzero_idx.size > 0:
        mu_obs    = at.take(mu_all, nonzero_idx)
        if hasattr(alpha_all, "shape"):
            alpha_obs = at.take(alpha_all, nonzero_idx) if alpha_all.ndim != 0 else alpha_all
        else:
            alpha_obs = alpha_all

        nb_dist   = pm.NegativeBinomial.dist(mu=mu_obs, alpha=alpha_obs)
        logp_val  = pm.logp(nb_dist, effective_cases[nonzero_idx])
    ...
    n_pred = n_int.astype(float)
    n_pred[zero_mask] = LARGE_N_FOR_PRED
    mu_pred = pi * n_pred + EPS

    # alpha for predictive (broadcast as needed)
    alpha_pred = alpha_all  # scalar or vector OK

    count_pred = pm.NegativeBinomial(name=f"p_count_{data_type}", mu=mu_pred, alpha=alpha_pred)
    pm.Deterministic(name=f"p_pred_{data_type}", var=count_pred / n_pred)
